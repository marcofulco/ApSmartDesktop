query['aspettoBeni.html'] = new Array;
query['aspettoBeni.html']['oggetti'] = new Array;
query['aspettoBeni.html']['oggetti']['txtID'] = "id";
query['aspettoBeni.html']['oggetti']['txtDescrizione'] = "descrizione";
...
var skUtente=sessionStorage.getItem("skVarie.aspettoBeni");
...
function carDatiAnag() {
  ...
  jSonCli = window.sessionStorage.getItem("skVarie.aspettoBeni");

  if (jSonCli != undefined) {
    jSonCli = JSON.parse(jSonCli);
    console.log(jSonCli[0])
    popolaFormModificaDati(jSonCli[0], "aspettoBeni.html");
...
function esci() {
  window.open("listeVarie.html?tabella=aspettoBeni", "_self");
}
...
function Salva() {
  ...
  for (x in query["aspettoBeni.html"]["oggetti"]) {
    jSon[query["aspettoBeni.html"]["oggetti"][x]] = recuperaValueElemento(x);
  }
  ...
  var parametri = { tipoRisposta: tipo, tipoSalva: "aspettoBeni", dati: jSon };
