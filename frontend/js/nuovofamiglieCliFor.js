query["nuovofamiglieCliFor.html"] = new Array();
query["nuovofamiglieCliFor.html"]["oggetti"] = new Array();
query["nuovofamiglieCliFor.html"]["oggetti"]["txtID"] = "id";
query["nuovofamiglieCliFor.html"]["oggetti"]["txtDescrizione"] = "DESCRIZIONE";
query["nuovofamiglieCliFor.html"]["oggetti"]["cmbFamRif"] = "FAMRIF";
query["nuovofamiglieCliFor.html"]["oggetti"]["txtPrezzoLav"] = "PREZZOLAV";
...
var skUtente=sessionStorage.getItem("skVarie.famiglieCliFor");
...
window.addEventListener("load", function (event) {
  setTimeout(function () {
    ...
    div.innerHTML += "FAMIGLIE CLIENTI/FORNITORI";

    recuperaParametri();
    avviaCarDati("cmbFamRif");
  }, 50);
});
...
function carDatiAnag() {
  jSonCli = window.sessionStorage.getItem("skVarie.famiglieCliFor");

  if (jSonCli != undefined) {
    jSonCli = JSON.parse(jSonCli);
    popolaFormModificaDati(jSonCli[0], "nuovofamiglieCliFor.html");
  } else {
    idModifica = 0;
  }
}
...
function esci() {
    if(typeof modElectron!='undefined' && modElectron==true){
        location.href="listaVarie.html?tabella=famiglieCliFor";
    }else{
        window.open("listeVarie.html?tabella=famiglieCliFor", "_self");
    }
...
function Salva() {
  ...
  for (x in query["nuovofamiglieCliFor.html"]["oggetti"]) {
    jSon[query["nuovofamiglieCliFor.html"]["oggetti"][x]] = recuperaValueElemento(x);
  }
  ...
  var parametri = { tipoRisposta: tipo, tipoSalva: "famiglieCliFor", dati: jSon };
...
function elaboraRispostaSalvaCarrello(res) {
  ...
  if (data[0] == 0) {
    attivaAlert(0, "Errore durante il salvataggio della Famiglia Cli/For", "fineSalva");
    return "";
  }
