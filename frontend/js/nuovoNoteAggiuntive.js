query['noteAggiuntive.html'] = new Array;
query['noteAggiuntive.html']['oggetti'] = new Array;
query['noteAggiuntive.html']['oggetti']['txtID'] = "id";
query['noteAggiuntive.html']['oggetti']['txtDescrizione'] = "descrizione";
query['noteAggiuntive.html']['oggetti']['sclNrNota'] = "nr_nota";

var parametriNC = {
  obbligatori:
    "txtDescrizione;nr_nota",
  alternativi: "",
  alternativi1: "",
  nascosti: "",
};

var skUtente=sessionStorage.getItem("skVarie.noteAggiuntive");
...
function carDatiAnag() {
  ...
  jSonCli = window.sessionStorage.getItem("skVarie.noteAggiuntive");

  if (jSonCli != undefined) {
    jSonCli = JSON.parse(jSonCli);
    console.log(jSonCli[0])
    popolaFormModificaDati(jSonCli[0], "noteAggiuntive.html");
...
function esci() {
    if(typeof modElectron!='undefined' && modElectron==true){
        ...
        window.open("listeVarie.html?tabella=noteAggiuntive", "_self");
    }
...
function Salva() {
  ...
  for (x in query["noteAggiuntive.html"]["oggetti"]) {
    jSon[query["noteAggiuntive.html"]["oggetti"][x]] = recuperaValueElemento(x);
  }
  ...
  var parametri = { tipoRisposta: tipo, tipoSalva: "noteAggiuntive", dati: jSon };
